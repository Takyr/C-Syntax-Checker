...
int main(){
	...
	while(fgets(str1,100,input)!=NULL){
		len=strlen(str1);                                           //Chieu dai cua line
		found=search(str1,len);                                     //Vi tri cua comment
		if (str1[0]!='\n'&&found==-1){
			str[i].lineno=i+1;
		    str[i].linelen=len;
	...
	printf("1. Kiem tra dau ngoac {} va () \n\n\n\n");
	bracket(str,totallen);
	
	printf("2. Cac bien (variable)duoc su dung\n\n\n\n");
	keyword(str,totallen);
	printf("3. Cac ham (function) )duoc su dung\n\n\n");
	builtinfunction(str,totallen);
	printf("4. Cac ham duoc su dung + ham Main() \n\n\n\n");
	function_and_prototype_count(str,totallen);
	printf("5. So luong bien (variable) duoc su dung\n\n\n");
	varcount(str,totallen);
	
	
	printf("6. Kiem tra cu phap:\n\n");
	fcheck(str,totallen);
	printf("7. Tong so dong (lines) cua chuong trinh\n\n\n");
	printf("So dong: %d",totallen);
	return 0;
}
...
void bracket(fileline arr[],int len){
	...
	while(*p>0)  {   printf("line %d: { cao hon\n",linea1[--a]) ; *p-=1;     }
	while(*p<0)  {   printf("line %d: } cao hon\n",linea2[--a2]);*p+=1;      }
	while(*(p+1)>0){ printf("line %d: ( cao hon\n",lineb1[--b]);*(p+1)-=1;   } 
	while(*(p+1)<0){ printf("line %d: ) cao hon\n",lineb2[--b2]);*(p+1)+=1;  }
	while(*(p+2)>0){ printf("line %d: [ cao hon\n",linec1[--c]); *(p+2)-=1;  }
	while(*(p+2)<0){ printf("line %d: ] cao hon\n",linec2[--c2]);*(p+2)+=1;  }
}
...
void builtinfunction(fileline arr[],int len){ 
	...
	printf("Tong so luong cac function: %d\n",total);
}
...
void function_and_prototype_count(fileline arr[],int len){
	...
	printf("Tong so luong function + main(): %d\n",total);
	...
	printf("8. Kiem tra cac ham nguyen mau (Prototype Function)\n\n\n\n");
	
	printf("Tong so cac ham nguyen mau: %d\n",fun);
	
	if(fun<total-1)printf("Ham gap loi\n");
	else printf("Cac ham nguyen mau\n");
}
void varcount(fileline arr[],int len){
	char str[100],a[100];
	...
	printf("10. Bo nho duoc su dung boi Variable\n\n\n");
			...
			if (str[j]==' '||str[j]=='('|| str[j]=='\0'||str[j]=='\t'){
				a[k]='\0';
					if (strcmp(a,sync[0])==0){
						...
						if(f)printf("Loi Trong Cau Lenh; ---> line no: %d\n",arr[i].lineno);
					} 
